...
bot.kommentarTillBot = function(data) {
  ...
  // debug
  console.log('.kommentarTillBot fired');
  ...
  if (this.botInComment(comment) && this.authorIsNotBot(authorId)) {
    ...
    // debug
    console.log('.botInComment && authorIsNotBot are ok');
    
    if (/sekreterare/i.test(comment)) {
      
      // debug
      console.log('/sekreterare/ matched');
      
      answer += this._sekreterare() + " är sekreterare denna veckan."
